                    ...
                    <section>
                        <div className="max-w-7xl mx-auto px-4 py-16">
                            <div className="flex flex-col md:flex-row items-start md:space-x-8">
                                ...
                                <div className="md:w-1/2 w-full text-secondary-light mb-6 md:mb-0 text-secondary">
                                    ...
                                    {item.title === 'ALLERGY TEST' ? <>
                                        <button className="bg-primary-dark text-white font-semibold p-4 px-10 rounded-xs hover:bg-primary-dark transition-colors duration-200 mb-6"
                                            onClick={() => { window.location.href = "mailto:info@medilab.je" }}>
                                            Contact Us
                                        </button>
                                        <p className="mb-4 text-sm md:text-base">
                                            <b>Medilab</b> is proud to offer comprehensive allergy testing services to help patients identify their triggers, manage symptoms effectively, and take proactive steps towards better health.
                                        </p>
                                        <p className="mb-4 text-sm md:text-base">
                                            Allergy testing is a vital tool for identifying substances (allergens) that trigger abnormal immune reactions in the body. These reactions can range from mild discomfort to severe, potentially life-threatening responses such as anaphylaxis. By pinpointing specific allergens whether they are foods, environmental factors, insect venoms, or medications—healthcare providers can:
                                        </p></> : ""}
                                    <p className="mb-4 text-sm md:text-base">
                                        {item.detail}
                                    </p>
                                    {/* Points */}
                                    {item.points?.length ? <>  <h3 className="font-semibold mb-2">Points:</h3>
                                    ...
                                    {/* Book Now Button */}
                                    {item.title !== 'ALLERGY TEST' ? <> <button className="bg-primary-dark text-white font-semibold p-4 px-10 rounded-xs hover:bg-primary-dark transition-colors duration-200 mb-6">
                                        Book Now
                                    </button>
                                ...
                                <div className="md:w-1/2 w-full  text-secondary-light">
                                    <div className="w-full  text-secondary-light">
                                        ...
                                        <div>
                                            {item.Why_get_tested?.length ? <> <h3 className="font-semibold mb-2 text-md">Why get tested?</h3>
                                                {item.Why_get_tested.map((i, testkey: number) => {
                                                    return <p key={testkey} className="mb-4 text-sm md:text-base">
                                                        {i}
                                                    </p>
                                                })}</> : ""}
                                            {item.results_mean?.length ? <>{item.results_mean && <><h3 className="font-semibold mb-2 text-md">What do the results mean?</h3>
                                                {item.results_mean.map((i, testkey: number) => {
                                                    return <p key={testkey} className="mb-4 text-sm md:text-base">
                                                        {i}
                                                    </p>
                                                })}
                                                {item.title === 'MEDIDISCOVER' ?
                                                    <ul className="list-disc pl-6 space-y-3 text-gray-800">
                                                        <li>
                                                            <span className="font-semibold text-primary-dark">Heart Health – </span>
                                                            Find out your levels of both good cholesterol (HDL Cholesterol) and bad cholesterol
                                                            (LDL Cholesterol) as well as Total Cholesterol, Triglycerides and Total Cholesterol/HDL Ratio.
                                                            Higher than normal levels of LDL cholesterol may make you more likely to have heart problems or a stroke.
                                                        </li>
                                                        <li>
                                                            <span className="font-semibold text-primary-dark">Diabetes Health – </span>
                                                            HbA1c levels are measured to provide an overall picture of average blood sugar levels
                                                            over a period of weeks/months. HbA1c levels can be used to indicate prediabetes or diabetes.
                                                        </li>
                                                        <li>
                                                            <span className="font-semibold text-primary-dark">Full Blood Count</span>
                                                        </li>
                                                    </ul> : ""}
                                            </>}</> : ""
                                            }
                            ...
                            {item.packages ? <div className="flex flex-col md:flex-row items-start md:space-x-8">
                                <div className="md:w-1/2 w-full  text-secondary-light"> {item.title !== 'ALLERGY TEST' ? <h2 className='text-xl font-bold mb-4'>What tests are included?</h2> : ""}
                                    {item.packages.slice(0, item.slice).map((section, sectionIdx) => (
                                        <div key={`${section.title || section.category}-${sectionIdx}`} className='mb-3'>
                                            ...
                                            {section?.stages ? section.stages.map((stage: any, idx: number) => (
                                                ...
                                                {(section?.tests || section?.items).map((test: any, idx: number) => (
                                                    <li key={idx}>{test}</li>
                                                ))}
                                            </ul>}
                                ...
                                <div className="md:w-1/2 w-full  text-secondary-light">
                                    {item.packages.slice(item.slice).map((section, index) => (
                                        <div key={`${section?.title || section?.category}-${index}`} className='mb-3'>
                                            <h3 className='font-semibold'>{section?.title || section?.category}</h3>
                                            {section?.stages ? section.stages.map((stage: any, idx: number) => (
                                                <div key={idx} className="mb-4">
                                                    <h3 className="font-semibold italic">
